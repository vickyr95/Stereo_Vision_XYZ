...
cap = cv.VideoCapture('/home/ramesh/Vicky/stereo_vision/new_camera_data/stereo_camera_test_230208/80cm/stereo_80cm_ball_230208_0218pm.avi')
...
left_v = cv.VideoWriter('left.avi', cv.VideoWriter_fourcc(*'XVID'), fps, (width, height))
...
try:
    while True:
        ...
        right_frame=frame[0:frame.shape[0],(frame.shape[1]//2):frame.shape[1]]
        ...
        if any(ball_xyz) != 0: 
            ...
            Lx=_map(bX,-60,60,-50,50)
            Ly=_map(bY,oY,50,0,100)
            
            #ball reference from origin
            if camera_to_world == 0:
                ball_left_xyz=get_xyz((lcenter[0]-lw,lcenter[1]),(rcenter[0]-w,rcenter[1]))
                ball_right_xyz=get_xyz((lcenter[0]+lw,lcenter[1]),(rcenter[0]+w,rcenter[1]))
        ...
        if write_video: left_v.write(left_frame)
